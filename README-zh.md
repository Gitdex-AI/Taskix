...
Taskix 是一个本地优先的控制台，用来让 Codex 驱动的软件团队围绕 GitHub issue 和 pull request 工作。
...
Taskix 使用本地 `codex` CLI、本地 `gh` CLI、GitHub issues/PRs/labels，以及本地 SQLite 状态。它的设计原则是让后端保持确定性：服务端负责任务排队、上下文传递、状态存储和 GitHub 同步；需要判断的工作交给 Codex agent。
...
- 绑定 GitHub 仓库的项目管理。
- 本地 SQLite 存储，路径为 `data/taskix.sqlite`。
...
- 开发 worktree 隔离，位于 `data/taskix-workspaces/`。
- 开发 PR 创建/恢复，以及 QA 失败后的重试。
- QA 验证、评论和 `taskix:qa-*` 标签。
- 合并前的架构师代码审核。
...
- Self-update UI，可拉取/构建最新 Taskix 代码，并在配置后请求服务重启。
- 可选 Telegram PM 入口。
...
Taskix 会把它记录为一个需求。新的需求可以随时进入系统，不需要等待已有需求完成；多个 workflow 可以并行存在。
...
Taskix 创建和读取 labels，用于表示持久的 GitHub 工作流状态。
...
Labels 应通过 Taskix 修改，或者由理解工作流的操作人员手动修改。UI 把 GitHub labels 作为可见 badge 来源；本地 job 状态只控制按钮 loading 和实时执行状态。
...
```text
data/taskix.sqlite              SQLite 运行时数据库
data/taskix-workspaces/         每个 agent 的 Git worktree
...
Taskix 要求宿主机上的 `codex exec` 和 `gh` 操作可以正常工作。
...
5. 允许 Taskix 更新仓库中的 `AGENTS.md` workflow 区块。
6. 打开项目页面，从 PM 聊天开始。

Taskix 会保留 `AGENTS.md` 中托管区块以外的内容。
