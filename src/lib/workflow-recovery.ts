...
export function recoveryReasonForJobs(jobs: RecoveryJob[]): string | null {
  if (jobs.some((job) => job.status === "failed")) return "A planning job failed. Retry the failed job, or refresh GitHub triage if issues were created before the failure.";
  if (jobs.some((job) => job.status === "running")) return "Planning is running. If the status does not change after several minutes, refresh GitHub triage or retry after it is marked failed.";
  return null;
}
...
export function recoveryReasonForDeveloperStep(
  ...
  if (issues.some((issue) => issue.branch && !issue.prUrl)) return "Developer work has a branch but no recorded PR. Refresh GitHub triage to recover a PR that was created or finish PR creation manually.";
  if (issues.some((issue) => hasAnyLabel(issue, ["gd:dev", "gitdex:dev-running"]) && !issue.prUrl)) return "Developer work is marked running without a PR. Refresh GitHub triage to detect a partially completed PR, or retry the developer job.";
  return null;
}
...
export function recoveryReasonForQaStep(workflows: RecoveryWorkflow[], sessions: RecoverySession[]): string | null {
  ...
  if (issues.some((issue) => (issue.prUrl || issue.prState?.toUpperCase() === "OPEN") && !hasAnyLabel(issue, ["gd:qa", "gd:review", "gd:merge", "gd:fix", "gd:architect", "gd:blocked", "gitdex:need-qa", "gitdex:qa-running", "qa-passed", "gitdex:qa-passed", "qa-failed", "gitdex:qa-failed", "gitdex:spec-blocked", "gitdex:env-blocked", "gitdex:ready-to-merge"]))) {
    return "A PR is open but QA labels are missing. Refresh GitHub triage to recover labels, or request QA before this workflow can move forward.";
  }
  if (issues.some((issue) => issue.prUrl && hasAnyLabel(issue, ["gd:qa", "gitdex:need-qa", "gitdex:qa-running"]))) {
    return "A PR is waiting on QA labels. After QA finishes, refresh GitHub triage so Gitdex can move the workflow to pass/fail or merge readiness.";
  }
  return null;
}
...
export function recoveryReasonForMergeStep(workflows: RecoveryWorkflow[]): string | null {
  ...
  if (issues.some((issue) => hasAnyLabel(issue, ["gd:review", "gd:merge", "qa-passed", "gitdex:qa-passed", "gitdex:ready-to-merge"]) && issue.prState !== "MERGED")) {
    return "QA has passed and the PR is ready. Merge from this step, then refresh GitHub triage if the issue or PR state does not update.";
  }
  return null;
}
